...
<div class="d-sm-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-2 mb-sm-0">Jadwal Pelajaran</h1>
</div>
<div class="row">
	<div class="col-12">
		<div class="card">
                ...
                <div class="table-responsive">
                    <table class="table table-sm table-bordered">
                        <thead class="bg-light">
                            ...
                            <tr>
                                @for($i=1;$i<=6;$i++)
                                <th>{{ $anggota_rombel->rombel->nama }}</th>
                                @endfor
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($jp as $j)
                            <tr>
                                <td><small>{{ $j->jam_mulai }}-{{ $j->jam_selesai }}</small></td>
                                @for($i=1;$i<=6;$i++)
                                        <?php $jadwal = \App\Models\Jadwal::where('jp_id','=',$j->id)->where('rombel_id','=', $anggota_rombel->rombel->id )->where('ta_id','=',session()->get('taa'))->where('hari','=',$i)->first(); ?>
                                        @if($jadwal)
                                            <td>
                                                <a href="#" class="btn-detail" data-bs-toggle="tooltip" title="Detail Jadwal" data-id="{{ $jadwal->id }}" data-hari="{{ $i }}" data-jam="{{ $j->id }}" data-rombel="{{ $anggota_rombel->rombel->id }}" data-mapel="{{ $jadwal->gurumapel_id }}">{{ $jadwal->guru_mapel->mapel->kode }}</a>
                                            </td>
                                        @else
                                            <td>-</td>
                                        @endif
